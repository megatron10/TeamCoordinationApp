...
async def inform_contacts(uid, status):
    ...
    if len(to) > 0:  # asyncio.wait doesn't accept an empty. use list len(to) > 0 and
        await asyncio.wait([UID_to_USER[contact_id].send(inform_status_msg(uid, status)) for contact_id in to])
    return 0
...
async def communicate(websocket, path):
    ...
    request. The first message must have action field set to connect. All fields
    are at all times mandatory.
    On connecting the server will send the client a list of online users in the org.
    The server shall send messages to inform about any change in online status of users.
    '''
    ...
    try:
        async for message in websocket:
            ...
            if not check_valid_sid(data['uid'], data['sid']):
                ...
                break
            # connect and get list of online contacts, after this client will be updated whenever a contact changes status
